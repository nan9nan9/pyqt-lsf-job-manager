...
```
High-level (기본 권장)     JobSet 핸들 — js = mgr.submit(...); js.updated.connect(...)
Low-level  (고급/대시보드)  전역 Facade Signal — mgr.jobset_updated(jsid, ...) 등 (v6 유지)
```

### 1.1 High-level: JobSet 핸들

- `mgr.submit(...)`은 jobset_id 문자열이 아니라 **JobSet 핸들(QObject)**을 반환
- 핸들은 해당 JobSet 전용 Signal을 가짐 → jsid 필터링 불필요
- 라이프사이클 자동화:
...
class JobSet(QObject):
    ...
    def cancel(self): ...              # 진행 중 submit 중단
...
- 핸들 재획득: `mgr.jobset(jobset_id) -> JobSet` (복원/검색 결과에서)
- 핸들 수명: manager가 소유, JobSet close/삭제 시까지 유효.
  파괴된 핸들 접근 시 `JobSetClosedError`

### 1.4 Low-level Facade (v6 유지, 요약)

여러 JobSet 통합 대시보드용. 전역 Signal:
`submit_started/progress/finished`, `jobset_updated`, `jobs_updated`,
`job_lost`, `kill_finished`, `error_occurred` (모두 jobset_id 인자 포함).
핸들 Signal은 이 위에 얹힌 편의 계층 (동일 이벤트 이중 발행).
...
| 용어 | 코드 명칭 | 정의 |
|---|---|---|
| **JobSet** | `jobset_id`, `JobSet` 핸들 | 본 라이브러리의 논리적 job 묶음. 모든 기능의 기본 단위 |
...
- 복원은 자동 수행 안 함 — 앱이 orphan 목록 보고 결정.
  `mgr.recover_jobset(id) -> JobSet` 핸들 반환 (High-level 통합)
...
- **FR-3 Kill**: 전략 우선순위 ①`bkill -g` ②array ③`-J` 패턴 ④chunking,
  부분 kill(`only_state`), verify 옵션. **[v7] `js.kill()` 핸들 메서드 추가**
...
├── handle.py            # [v7] JobSet 핸들(QObject) — 전용 Signal, 위임 메서드
└── manager.py           # LsfJobManager: Facade Signal + 핸들 발급 + 옵션 해석
                         #   + AUTO-1~4 + shutdown
```
...
16. **[v7] 핸들 Signal**: js.updated가 해당 JobSet 이벤트만 수신
...
8. `manager.py` + `handle.py` — Facade/핸들/AUTO-1~4 + pytest-qt 테스트
