...
`create_jobset` 에 넘긴 **각 wrapper 커맨드를 그대로 실행하고, 그 결과의
...
| `pre_submit_finished` | `(jobset_id, ok)` | `pre_submit` 게이트 종료. `ok=True`면 이어서 `submit_started` |
...
js = mgr.create_jobset(["customwrapper_sub -q normal run_0.sp"])
mgr.submit(js)
