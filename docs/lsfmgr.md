...
| `submit_finished` | `(jobset_id, SubmitReport)` | 위 submit 계열 · `js.submit`(재제출) 완료 · `cancel_submit`(중단 마무리) |
...
| `js.submit_progress` | `(done, total)` | `submit_progress` | submit 계열 · `js.submit` |
| `js.submit_finished` | `(SubmitReport)` | `submit_finished` | submit 계열 · `js.submit` · `js.cancel` |
...
js.kill(); ...kill_finished 대기...

# 2) 다시 돌릴 job 을 같은 merge_id 로 담은 jobset 을 만들어 흡수
fix = mgr.create_jobset(label="fix")
fix.create_job("customwrapper_sub -q long a.sp", merge_id="case-a")
if js.can_merge(fix):
    js.merge_from(fix)     # case-a 가 CREATED 로 교체(물리 키 유지),
...
- `js.submit()` 은 **전 job 재제출** 이다 — 전원 비활성(CREATED/DONE/EXIT/
...
- **폴링이 돌고 있어야 동작한다** — handler 는 폴링 사이클에 tie 돼 있고, 첫 실행은
  ...
  자동으로 돈다.
- `js.submit()` 으로 재실행되는 job 은 진행 상태가 **자동 재무장**되어 새 실행에서
  다시 돈다(§2.4).
