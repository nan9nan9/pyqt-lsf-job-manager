# lsfmgr 사용 가이드 (wrapper 제출 + Signal)

실제 환경에서는 job 마다 `customwrapper_sub` 같은
**툴 전용 제출 wrapper** 를 쓴다. 각 wrapper 는 툴에 맞는 인자 처리·전처리를 한
뒤 내부에서 `bsub` 를 호출한다. lsfmgr 는 이 구조를 그대로 지원한다 —
`create_jobs` 에 넘긴 **각 wrapper 커맨드를 그대로 실행하고, 그 결과의
`Job <id>` 로 job 을 관리**한다.
...
- **입력**: wrapper 커맨드들의 리스트. 각 항목이 job 1개다.
  ...
  - 토큰 리스트 `["customwrapper_sub", "-q", "normal", "run_0.sp"]` → 셸 파싱 없이 그대로(공백/‏특수문자 안전)
...
| `js.submit_progress` | `(done, total)` | `submit_progress` | submit 계열 · `mgr.submit(js)` |
| `js.submit_finished` | `(SubmitReport)` | `submit_finished` | submit 계열 · `mgr.submit(js)` · `mgr.cancel_submit(js)` |
| `js.jobset_updated` | `(summary)` | `jobset_updated` | submit 완료(초기 PEND) · `mgr.start_polling(js)` · `mgr.query_once(js)` |
| `js.jobs_failed` | `([JobRecord])` | (파생) | submit 완료 시 `SUBMIT_FAILED` + polling 중 실패 상태 전이분 |
| `js.kill_started` | `()` | `kill_started` | `mgr.kill(js)` · `mgr.kill_jobs(js, keys)` 접수 즉시(동기) |
...
> 특정 JobSet 의 일부 job 만 죽이려면 **`mgr.kill_jobs(js, job_keys)`** (또는
> 원시 id 로 `mgr.kill_jobs(ids, jobset_id=...)`)를 쓰면 `js.kill_finished` 중계·verify 까지 모두 켜진다.
...
js = mgr.create_jobset()
mgr.create_jobs(js, ["customwrapper_sub -q normal run_0.sp"])
mgr.submit(js)
mgr.start_polling(js)                # 이후 상태는 jobset_updated 로 도착
# ... 나중에 ...
...
`resubmit_jobs` 는 제거되었다(내부 kill→재제출 파이프라인 관리 문제).
재실행은 **데이터 조작 + 일반 submit** 으로 표현한다 — job control 은
앱(GUI)이 직접 갖는다:
...
mgr.create_job(fix, "customwrapper_sub -q long a.sp", merge_id="case-a")
...
- **폴링이 돌고 있어야 동작한다** — handler 는 폴링 사이클에 tie 돼 있고, 첫 실행은
  다음 폴링 사이클이다(`mgr.query_once(js)` 로 즉시 1회 유도 가능). `auto_poll`(기본)이면
  자동으로 돈다.
...
- wrapper 로 제출한 job 은 그룹/이름 부착물을 **사용하지 않는다**. 관리는
  커맨드 출력에서 얻은 **job_id 로만** 한다.
...
기반으로 관리하길 원할 때 `create_jobs(..., wrapper=False)` 로 쓴다.

```python
mgr = LsfJobManager(bsub_path="bsub")          # 또는 ["customwrapper_sub", "--proj", "X"]
js = mgr.create_jobset()
mgr.create_jobs(js, ["mytool run_1.sp", ...], wrapper=False)  # bsub 인자 조립
mgr.submit(js)
```
