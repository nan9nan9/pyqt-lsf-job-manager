...
- (후속 검토) 조회의 대상 스냅샷과 조회원 관심 등록 사이에 삭제·교체·재제출로
  ...
  `test_stale_ids.py`.
- (직접 조회 보완) 관심 ID 정리를 `JobsetQuerier.query_ids`로 모아 폴링·전역
  kill 검증·timeout 후 조회에 공통 적용한다. 조회 성공·실패와 무관하게 Store에
  없는 ID를 정리하므로 미등록 ID나 kill 중 삭제된 ID도 남지 않는다. 같은 ID를
  조회 중인 호출 수를 잠금 아래 관리해, 마지막 호출이 결과를 읽은 뒤 정리한다.
  조회 자체는 잠금 밖에서 병렬 실행하며, Store에 남아 있는 작업의 증분 원장은
  보존한다. 회귀: 직접 조회 경로·삭제·조회 장애 12개 조합, 추적 중인 원장 보존,
  겹친 조회의 결과 보존(`test_stale_ids.py`).
- (정리 비용 보완) `query_ids`의 정리 지점은 유지하되, 폴링은 `jobset_id`와
  이미 선택한 `job_keys`를 넘겨 `get_jobs_by_keys`로 현재 레코드만 확인한다.
  전체 Store뿐 아니라 같은 jobset의 완료 레코드도 다시 훑지 않는다.
  소속만 아는 kill timeout 조회는 해당 jobset을 읽고, 소속을 모르는 직접 조회만
  `find_jobs`를 사용한다. 조회 중 jobset이 삭제돼도 관심 정리는 완료한다.
  회귀: 폴링의 전역 검색·불필요한 전체 읽기 금지, jobset 삭제와 조회 실패
  4개 조합(`test_stale_ids.py`).
...
- 직접 조회 관심 ID 정리 보완: 회귀 테스트 **14개 추가**, 관련 **287개 통과**,
  당시 전체 **1,083개 통과** (231.85초).
- 정리 비용 보완: 회귀 테스트 **5개 추가**, 관련 **206개 통과** (33.98초),
  최종 전체 **1,088개 통과** (243.31초).
- `git diff --check` 통과.

정리 비용 실측(Python 3.12.1, 20개 jobset × 5,000건 = 10만 레코드,
조회 대상 500개, 준비 호출 5회 후 50회씩 7묶음의 호출당 중앙값):

| 측정 대상 | 호출당 비용 |
|---|---:|
| `find_jobs`로 ID 500개 검색 | 12.7061 ms |
| `get_jobs_by_keys`로 key 500개 읽기 | 0.0591 ms |
| `query_ids`: 이전 전역 정리 경로 | 13.3257 ms |
| `query_ids`: 소속 jobset만 읽는 경로 | 0.4768 ms |
| `query_ids`: 폴링 대상 key만 읽는 경로 | 0.2974 ms |

`query_ids` 측정은 외부 조회를 즉시 반환하는 대역으로 바꿔 네트워크·콜백 비용을
제외했다. 참조 카운트와 정리용 집합 계산은 포함하며, key 경로에는 폴링의
key 목록 생성 비용도 포함한다. 전체 폴링 사이클이나 실제 Store 잠금 점유
시간을 측정한 수치는 아니다.
