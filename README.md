...
mgr = LsfJobManager()
js = mgr.submit([f"mytool run_{i}.sp" for i in range(5000)])
...
### 2.1 wrapper 커맨드로 제출 — `submit_wrapper` (예: `customwrapper_sub`)
...
js = mgr.submit([f"mytool tt_{i}.sp" for i in range(1000)], mode="array")
```
...
### 툴별 제출 wrapper (`customwrapper_sub` / `customwrapper_sub` / `customwrapper_sub` / `customwrapper_sub`)

2.1절의 `submit_wrapper` 구조를 실제로 테스트할 수 있도록, 커스텀 툴 전용 제출
