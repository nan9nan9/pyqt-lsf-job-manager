...
- **지금 누가 답하고 있나** — `mgr.status_fetcher_state()`가 `FetcherState`를
  ...
  판정은 마지막으로 **끝난** 조회 기준입니다 — 콜백이 안 돌아오면(미회수)
  판단 보류가 확정되는 시점(`query_timeout_s` 초과)에 DOWN으로 반영됩니다.
  예비가 없어도 유효합니다(주 콜백 하나의 생사를 PRIMARY/DOWN으로 답함).
...
콜백 반환 JSON
  ...
  │              ※ poll_runtime_updates=False(기본)면 **건너뜁니다** — 그 값을
  │                폴링이 어차피 안 쓰는데 원장을 매번 전수 스캔할 이유가 없다
  │
...
— 다른 툴·다른 세션의 job은 ①에서 걸러집니다. 등록은 파싱보다 **먼저** 일어나므로
갓 제출한 job이 첫 조회에서 누락되지 않습니다(조회가 이미 떠 있는 동안 들어온
등록도 포함 — 필터 기준은 콜백이 돌아온 **직후**에 뜹니다).
...
보관 0건, 파싱 피크 61MB→26MB). 등록은 파싱보다 **먼저** 하므로 갓 제출된 job이
첫 조회에서 누락되지 않습니다.
...
- **레코드를 지우면**(`remove_jobs`/`clear_jobs`/`remove_jobset`) 그 job은 만료를
  기다리지 않고 즉시 원장에서 빠집니다. 만료는 DONE/EXIT만 보므로, 안 그러면
  강제 삭제된 RUN job이나 원장에 오른 적 없는 job(LOST/CANCELLED)이 jobset을
  만들고 지우기를 반복하는 세션에서 계속 쌓입니다.
...
> 실행시간은 실측이라 건드리지 않습니다. 이 재계산은 `poll_runtime_updates`가
> 켜져 있을 때만 돕니다 — 꺼져 있으면(기본) 폴링이 `run_time_s` 변화를 갱신
> 대상에서 빼므로 만들어 봐야 버려집니다(2만 건 기준 조회당 37ms→14ms).
...
사용자 명령에 대한 신호는 아래 순서로 전달됩니다. `jobs_updated`는 Store에
이미 반영된 전이의 스냅샷입니다. `js.jobs()`는 호출 시점의 현재 스냅샷이므로,
표시 지연을 꺼도 worker의 다음 갱신이 먼저 진행되면 신호보다 앞선 값을 볼 수
있습니다. 같은 실행에서 이미 전달한 revision보다 오래된 결과는 버립니다.
...
> `min_state_dwell_s`(§6.5)를 켜면 **`jobs_updated`만** 대기 중인 전이 수 ×
> dwell만큼 추가 지연될 수 있어, 완료 보고서보다 늦게 도착할 수 있습니다.
> 기본값(0)이면 이 표시 대기를 추가하지 않습니다.

완료 판정은 진행 중인 kill의 검증·마킹 결과가 전달될 때까지 보류한 뒤 다시
수행합니다. 전역 ID kill도 추적 중인 JobSet에 같은 규칙을 적용합니다.
전원 `killed=True`인 완료는 통지를 생략하며, 자연 종료나 혼합 완료는 통지합니다.
...
**표시만** 늦추는 기능입니다. 다른 신호·Store·라이브러리 내부 판정은 영향을
받지 않으며, 지연된 `jobs_updated`는 다음처럼 사용합니다:

- **pull은 현재 스냅샷** — 지연된 `jobs_updated` slot에서 `js.jobs()`를 pull하면
...
교체·삭제된 job의 표시 대기열은 비웁니다. 재제출은 이미 접수한 표시 대기열을
유지해 위 예의 순간 전이를 보여 줍니다. 재제출 후 뒤늦게 접수되는 이전 실행의
조회·제출 결과는 무시합니다.
