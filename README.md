...
| `submit_finished_on_gate_reject` | True | 생성자 | `pre_submit` 게이트가 False면 `submit_finished`(cancelled=N)도 발화. False면 종료는 `pre_processing_finished(False)`만 |
...
신호 순서는 **`pre_processing_started` → `pre_processing_finished(ok)` → (ok=True일 때만)
...
`pre_processing_finished(False)`만). 콜백이 **예외**를 던져도 제출하지 않으며 —
...
submit (mgr.submit(js) — 재제출 포함):
  (pre_processing_started → pre_processing_finished)          # pre_submit 게이트 지정 시
