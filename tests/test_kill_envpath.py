...
# ----------------------------------------------------------------------
def test_resubmit_envpath_kills_forwarded(qtbot, fake_lsf, config):
    fake_lsf.forward_needs_env = True
    mgr = LsfJobManager(store=InMemoryStore(), config=config, runner=fake_lsf,
                        collect_clusters=True)
    try:
        ...
        rec = js.jobs()[0]
        old_id = rec.job_id
        fake_lsf.jobs[str(old_id)].stat = "RUN"
        fake_lsf.jobs[str(old_id)].forward_cluster = "busan"
        mgr.querier.query(js.id)

        with qtbot.waitSignal(mgr.submit_finished, timeout=10000):
            js.resubmit_jobs([rec.job_key], envpath=CSHRC, verify=False)
        # 원 forward job이 sourced bkill로 죽고, 새 job만 살아있다
        assert fake_lsf.jobs[str(old_id)].stat == "EXIT", "원 job이 안 죽음(좀비)"
        new_id = js.jobs()[0].job_id
        assert new_id != old_id and fake_lsf.jobs[str(new_id)].stat == "PEND"
        assert any(CSHRC in c[2] for c in fake_lsf.calls_of("tcsh"))
    finally:
        mgr.shutdown()


def test_resubmit_without_envpath_leaves_zombie(qtbot, fake_lsf, config):
    """envpath 없이 forward job을 resubmit하면 원 job이 살아남는 문제 재현 —
    MC 환경에서 envpath를 줘야 하는 이유."""
    fake_lsf.forward_needs_env = True
    mgr = LsfJobManager(store=InMemoryStore(), config=config, runner=fake_lsf,
                        collect_clusters=True)
    try:
        ...
        mgr.querier.query(js.id)
        with qtbot.waitSignal(mgr.submit_finished, timeout=10000):
            js.resubmit_jobs([rec.job_key], verify=False)   # envpath 없음
        assert fake_lsf.jobs[str(old_id)].stat == "RUN"     # 좀비로 남음
    finally:
        mgr.shutdown()
