...
- C16-1: merge가 job을 **교체**해도 그 job_key의 handler 장부가 남았다.
  merge는 같은 merge_id면 물리 키(job_key)를 유지한 채 내용만 바꾼다(테이블
  행 연속성). 그런데 옛 실행의 _FINISHED가 그대로 남아, 그 키의 **새 job**에
  handler가 영영 침묵한다(_eval_record 첫 줄에서 걸러짐).
  흡수분이 CREATED가 되는 통상 경로에서는 재제출의 rearm이 덮어 가려져
  있었지만, force merge로 이미 RUN인 job을 흡수하면 재제출이 없어 바로 드러난다.
  → merge가 흡수분(changed)의 장부를 무효화한다. pacer/querier 보류분을
    _forget_paced로 정리하는 것과 같은 이유.
...
# C16-1
# ----------------------------------------------------------------------
def test_merge_replace_reruns_handler_without_resubmit(qtbot):
    """교체된 job에 handler가 다시 돈다 — 재제출(rearm) 없이.

    force merge로 **이미 RUN인** job을 흡수하면 그 job은 곧장 관찰 대상이
    되므로 재제출을 거치지 않는다. 장부가 안 지워지면 여기서 침묵한다.
    """
    fake = FakeLsf()
    mgr = _mgr(fake)
    seen = []
    ...
    try:
        ...
        # source: 같은 merge_id "a"를 제출해 RUN 상태로 만든 뒤 흡수
        src = mgr.create_jobset(["customwrapper_sub a2.sp"], merge_ids=["a"])
        with qtbot.waitSignal(mgr.submit_finished, timeout=10000):
            mgr.submit(src, auto_poll=False)
        fake.set_all("RUN")
        _poll(qtbot, mgr, src)                   # src 레코드를 RUN으로 갱신
        mgr.merge(tgt, src, force=True)          # 활성 job 흡수 → force

        rec = tgt.jobs()[0]
        assert rec.job_key == tgt_key            # 물리 키는 target 것 유지
        assert rec.state.value == "RUN"          # 내용은 source 것(재제출 없음)
...
def test_merge_does_not_touch_untouched_jobs(qtbot):
    """반대 방향 — merge에 관여하지 않은 기존 job의 장부는 건드리지 않는다.
    ...
    try:
        ...
        # 무관한 merge_id "b"를 흡수 — "a"는 손대지 않는다
