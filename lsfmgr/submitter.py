...
from .qt import (
    QObject, QRunnable, QThreadPool, QTimer, Signal, Slot, timer_ms,
)
...
class _SubmitContext:
    ...
    # 잠금 아래 발행한 신호의 direct 슬롯이 snapshot/cancel API로 재진입할 수 있어 RLock을 쓴다.
    lock: threading.RLock = field(default_factory=threading.RLock)
    #: 재시도 원장은 진행 카운터와 독립적으로 잠근다.
    #: 신호 발행 중 ctx.lock 경합을 피하며, 두 잠금은 중첩 취득하지 않는다.
    ...
    #: 선택 kill로 취소한 key. 잔여 task가 재제출하지 않도록 사이클 종료까지 유지한다.
...
class _SubmitTask(QRunnable):
    ...
    def _run(self):
        ...
        # 취소 판정과 inflight 등록을 원자적으로 묶어 kill의 대기 범위에서 빠지는 제출을 막는다.
        ...
        # 호출별 workers가 전역 상한보다 낮으면 추가 제한한다. 슬롯 대기 중에도 취소를 확인한다.
...
class BulkSubmitter(QObject):
    ...
    _retry_requested = Signal(object, str)    # 요청을 만든 context, 원장 key
    ...
    def __init__(self, store: JobSetStore, command: LsfCommand,
        ...
        # manager의 실효 workers를 공용 풀 상한으로 사용해 여러 jobset의 동시 제출도 제한한다.
    ...
    def resubmit_existing(self, jobset_id: str,
        ...
        if ctx.cancel_event.is_set():
            # barrier로 시작 전 취소된 경우 기존 job_id/이력을 리셋하지 않고 일괄 정산한다.
            # 게이트 신호가 없으므로 started → finished(cancelled) 짝을 직접 발행한다.
            self._safe_emit(self.started, jobset_id)
            self._gate_reject(ctx, finish=True)
            return False
        self._coord.start(_LaunchTask(self, ctx, keyed, pre_submit))
        return True

    def _launch_cycle(self, ctx: _SubmitContext, keyed: List) -> None:
        """제출 착수 본체 — **단계 순서 자체가 계약**이다. worker 스레드에서
        실행되며 pre_submit 유무와 무관하게 _LaunchTask가 호출한다. 착수 예외는
        여기서 실제 리셋분과 함께 _gate_fail로 정산한다.
        ...
        except Exception as e:  # 착수 실패도 이 cycle의 실제 리셋분만 정산한다.
            log.exception("제출 착수 실패: %s", ctx.jobset_id)
            self._gate_fail(ctx, repr(e), reset_recs)
    ...
    def cancel_submit(self, jobset_id: str) -> None:
        ...
        if ctx is None:
            return
        ctx.cancel_event.set()
        # 재시도 타이머 만료를 기다리지 않고 취소를 확정한다.
        # 이후 도착하는 재시도 요청도 _on_retry_requested에서 즉시 포기한다.
    ...
    def shutdown(self) -> None:
        ...
        # worker 종료 후 남은 재시도를 정산한다. 이벤트 루프 종료 뒤에는 타이머가 발화하지 않는다.
        for ctx in contexts:
            self._abort_retries_ctx(ctx, reason="SHUTDOWN")
    ...
    def _task_failed(self, ctx: _SubmitContext, job_key: str, attempt: int,
        ...
        if job_cancelled:
            # 제출 중 선택 취소된 job은 재시도하지 않고 CANCELLED/killed로 확정한다.
    ...
    def _schedule_retry(self, ctx: _SubmitContext, key: str,
        ...
        self._retry_requested.emit(ctx, key)

    @Slot(object, str)
    def _on_retry_requested(self, ctx: _SubmitContext, entry_key: str) -> None:
        ...
        def fire():
            ...
            if self._shutdown or ctx.cancel_event.is_set():
                ...
                return
            # backoff를 이미 기다린 재시도는 후속 작업보다 먼저 실행한다.
    ...
    def _emit_progress(self, ctx: _SubmitContext,
        ...
        # drain과 신호 발행을 같은 잠금으로 묶어 finished가 마지막 jobs_changed를 앞서지 않게 한다.
        with ctx.lock:
            if ctx.finished:
    ...
    def _finish_if_done(self, ctx: _SubmitContext, force: bool = False) -> None:
        with ctx.lock:
            ...
            # 다른 worker가 마지막 progress보다 먼저 완료를 감지할 수 있어 최종 (total, total)을 보장한다.
            # force 완료는 done < total일 수 있으므로 제외한다.
            if ctx.done >= ctx.total:
                self.progress.emit(ctx.jobset_id, ctx.total, ctx.total)
            # 마지막 상태 배치와 finished를 같은 잠금 안에서 순서대로 발행한다.
            if batch:                    # throttle 잔여 마지막 전이분
                self.jobs_changed.emit(ctx.jobset_id, batch)
            # 완료 신호 수신자가 로그를 바로 읽을 수 있도록 먼저 기록한다.
    ...
    def _finish_forced(self, ctx: _SubmitContext, *, failed: bool,
        ...
        with ctx.lock:
            if ctx.finished:
                return
            ctx.finished = True
            # 이미 정산한 단위의 분류를 보존하고 남은 수량만 확정한다.
    ...
    def _gate_fail(self, ctx: _SubmitContext, msg: str, reset_records=()) -> None:
        ...
        # 착수하지 못한 SUBMITTING만 해제한다. 이미 PEND가 된 작업은 CAS로 보호한다.
        stuck = []
        for r in reset_records:
            try:
                nr = self.store.transition(
                    ctx.jobset_id, r.job_key, JobState.SUBMIT_FAILED,
                    guard=lambda cur, target=r: (cur.same_execution(target)
                                                 and cur.state is JobState.SUBMITTING),
...
class _LaunchTask(QRunnable):
    """coordinator의 단일 착수 경로: 선택적 pre_submit → 레코드 리셋 → 제출."""

    def __init__(self, submitter: BulkSubmitter, ctx: _SubmitContext,
                 keyed: list, pre_submit):
        super().__init__()
        self.setAutoDelete(True)
        self.submitter = submitter
        self.ctx = ctx
    ...
    def run(self):
        ...
        if self.pre_submit is not None:
            ...
            if sub._shutdown or ctx.cancel_event.is_set():
                ...
                return
            try:
                ok = bool(self.pre_submit([
                    sub._item_command(item) for _key, item in self.keyed]))
            ...
            if not ok:
                sub._gate_reject(ctx, finish=sub.config.submit_finished_on_gate_reject)
                return
        sub._launch_cycle(ctx, self.keyed)
