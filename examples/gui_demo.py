...
class DemoWindow(QWidget):
    ...
    def add_jobs(self):
        """CREATE 단계 — create_job으로 누적. merge_id(논리 키)와 ud_data
        (실제 실행 정보)를 함께 싣는다. CREATED 행이 표에 바로 쌓인다."""
        n = len(self.js.jobs())
        self.js.create_jobs(
    ...
    def submit(self):
        ...
        같은 jobset 전이라 테이블 리셋 없음."""
        if not self.js.can_submit():
    ...
    def rerun_failed(self):
        ...
        if not failed or not self.js.can_submit():
            self.status.setText("rerun 불가 — 실패분 없음 또는 활성 job 존재")
            return
        fix = self.mgr.create_jobset(label="rerun-fix")
        fix.create_jobs([shlex.split(r.command) for r in failed],
                        merge_ids=[r.merge_id for r in failed],
                        ud_datas=[r.ud_data for r in failed])
        self.js.merge_from(fix)              # 같은 merge_id → CREATED로 교체
        self.js.submit(auto_poll=False)      # 전체 재실행
        self.js.start_polling(interval_s=1.0)
