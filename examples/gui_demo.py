...
바구니 구조: 앱 시작 시 create_jobset으로 빈 jobset(바구니)을 만들어
테이블을 **제출 전부터** 같은 핸들에 바인딩한다 — [Add]로 CREATED 행이
쌓이고, [Submit]이 같은 jobset을 전이시키므로 테이블 리셋이 없다.
...
class DemoWindow(QWidget):
    ...
    def __init__(self, mgr):
        ...
        for b in (self.btn_add, self.btn_submit, self.btn_kill,
                  self.btn_cancel, self.btn_resubmit):
            top.addWidget(b)
        ...
        # 바구니 선생성 — 제출 전부터 테이블이 이 핸들에 바인딩된다
    ...
    def add_jobs(self):
        """CREATE 단계 — 바구니에 누적. CREATED 행이 표에 바로 쌓인다."""
        n = self.table.rowCount()
        self.js.add_pending([wrapper(DEFAULT_WRAPPER, "-q", "normal",
                                     f"run_{n + i}.sp")
                             for i in range(N_JOBS)])
        self.status.setText(f"바구니에 {N_JOBS}건 추가 — 총 "
                            f"{len(self.js.jobs())}건 (아직 미제출)")

    def submit(self):
        """누적된 CREATED 전부 제출 — 같은 jobset 전이라 테이블 리셋 없음."""
        try:
            self.js.submit(auto_poll=False)
        except Exception as e:               # 빈 바구니 등
            self.status.setText(f"submit 불가: {e}")
            return
        self.js.start_polling(interval_s=1.0)  # 데모용 빠른 폴링 (기본 10s)
...
def main():
    ...
    if os.environ.get("LSFMGR_DEMO_AUTORUN") == "1":
        QTimer.singleShot(500, win.add_jobs)     # CREATE — 바구니 누적
